Function Invoke-VssRepair
{
    <#
        .SYNOPSIS
        Locates VSS Writer's in a failed state and attempts to fix the issue by restarting the service responsible for the writer.

        .DESCRIPTION
        Locates VSS Writer's in a failed state and attempts to fix the issue by restarting the service responsible for the writer.

        .PARAMETER Force
        Invoke-VssRepair will prompt for conformation when restarting a service.  With the force parameter the prompts will be supressed.

        .EXAMPLE
        Invoke-VssRepair
        Look for vss writers having problems and restart their related services.

        .EXAMPLE
        Invoke-VssRepair -Force
        Suppress any confirmation prompts regarding the service resets.
